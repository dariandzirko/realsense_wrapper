...
fn main() {
    if let Some(_) = depth_example() {
        print!("depth example passed")
    } else {
        println!("depth example failed");
    }
}

fn depth_example() -> Option<bool> {
    unsafe {
        ...
        rs2_config_enable_stream(
            config,
            rs2_stream_RS2_STREAM_DEPTH, //change this for Depth
            STREAM_INDEX as i32,
            WIDTH as i32,
            HEIGHT as i32,
            rs2_format_RS2_FORMAT_Z16, //find the correct format
            FPS as i32,
            &mut error,
        );
        ...
        if let Some(_) = error.as_ref() {
            println!("Error with depth streaming");
            return None;
        }
