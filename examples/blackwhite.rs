...
fn black_white_example() -> Option<bool> {
    unsafe {
        ...
        rs2_config_enable_stream(
            config,
            rs2_stream_RS2_STREAM_COLOR, //change this for Depth
            STREAM_INDEX as i32,
            WIDTH as i32,
            HEIGHT as i32,
            rs2_format_RS2_FORMAT_Y16, //find the correct format
            FPS as i32,
            &mut error,
        );
        if let Some(_) = error.as_ref() {
            println!("Error with black and white config");
            return None;
        }
        ...
        if let Some(_) = error.as_ref() {
            println!("Error with black and white streaming");
            return None;
        }
        ...
        for i in 0..num_of_frames {
            ...
            println!("Black and White frame arrived");
