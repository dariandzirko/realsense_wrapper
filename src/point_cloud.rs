...
impl PointCloud {
    ...
    pub fn new(depth_image: &ImageData, intrinsics: rs2_intrinsics) -> PointCloud {
        ...
        depth_image
            ...
            .for_each(|(index, chunk)| {
                let high_byte = (*chunk.first().unwrap() as u16) << 8;
                let low_byte = *chunk.last().unwrap() as u16;
                //What am i doing here, isn't this just a shift right 8 times?
                // let temp_data = (high_byte | low_byte) / u16::MAX * u8::MAX as u16;
                let temp_data = ((high_byte | low_byte) >> 8) as u8;
    ...
    pub fn to_xyz_file(&self) {
        let mut file = File::open("point_cloud.xyz").unwrap();

        self.points.iter().for_each(|(point)| {
            file.write_fmt(format_args!("{} {} {}", point.x, point.y, point.z));
        });
    }
}
